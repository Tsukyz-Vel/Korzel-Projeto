  ...
  return (
    <div className={`absolute flex flex-col items-center justify-between w-20 h-20 sm:w-24 sm:h-24 border-2 rounded-full p-1 sm:p-2 bg-black/60 backdrop-blur-sm transition-all duration-300 ${theme.border} ${theme.shadow} ${customClass}`}>
      <div className="flex-1 flex items-center justify-center w-full">
        <input type="number" value={value} onChange={(e) => onChange(Number(e.target.value))} className={`w-full text-center bg-transparent focus:outline-none text-2xl sm:text-3xl font-bold drop-shadow-md [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${theme.text}`} />
      </div>
