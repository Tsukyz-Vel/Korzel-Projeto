...
export default function Lobby({ handleEnterSession }) {
  ...
  const fetchData = async () => {
    try {
      const resCamp = await fetch('http://korzelapi.somee.com/api/campaigns', {
  ...
  const handleCreateCampaign = async (e) => {
    ...
    try {
      const res = await fetch('http://korzelapi.somee.com/api/campaigns', {
        method: 'POST',
 ...
 const handleJoinCampaign = async (e) => {
    ...
    try {
      const res = await fetch('http://korzelapi.somee.com/api/campaigns/join', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleDeleteCampaign = async (campaignId, campaignName) => {
    if (window.confirm(`Tem a certeza absoluta de que quer excluir a campanha "${campaignName}"? Todas as fichas, mapas e tokens associados a ela serão perdidos no vazio. Esta ação é irreversível.`)) {
      try {
        const res = await fetch(`http://korzelapi.somee.com/api/campaigns/${campaignId}`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${token}` }
        });
