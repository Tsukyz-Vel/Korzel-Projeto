...
export default function App() {
  ...
  const handleAuthSubmit = async (e) => {
    e.preventDefault();
    const url = authMode === 'login' ? 'http://localhost:5104/api/auth/login' : 'http://localhost:5104/api/auth/register';
    try {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data && data.length > 0) {
          ...
          const resCreate = await fetch(`http://localhost:5104/api/scenes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(firstScene) });
          if(resCreate.ok){
  ...
  const fetchAllCharacters = async () => {
    ...
    const url = currentCampaignId 
      ? `http://localhost:5104/api/characters/campaign/${currentCampaignId}` 
      : "http://localhost:5104/api/characters";
  ...
  const loadCharacterFromDb = async (idToLoad) => {
    try { const response = await fetch(`http://localhost:5104/api/characters/${idToLoad}`, { headers: getAuthHeaders() });
...
const executeRoll = (type, title, bonus, weapon = null, customExp = null) => { 
 ...
 const handleMapUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        img.onload = async () => {
          ...
          try {
            const res = await fetch(`http://localhost:5104/api/scenes/${gmActiveSceneId}/background`, {
              method: 'PUT',
  ...
  { 
    ...
    try {
      const res = await fetch(`http://localhost:5104/api/scenes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newScene) });
      if (res.ok) {
  ...
  const handleDeleteTokenFromScene = async (tokenId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5104/api/scenes/tokens/${tokenId}`, { method: 'DELETE' });
      if (res.ok) { 
  ...
  const handleDropOnMap = async (e) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:5104/api/scenes/tokens", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleMapMouseUp = async () => {
    if (draggingToken !== null) {
      ...
      if (token && currentCampaignId) {
        ...
        try {
          await fetch(`http://localhost:5104/api/scenes/tokens/${token.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(token) });
        } catch (e) {}
      }
    }
  ...
  const assignPermission = async () => { 
    ...
    if (playerName !== null) { 
      ...
      if (tokenToUpdate) {
        ...
        try { 
          await fetch(`http://localhost:5104/api/scenes/tokens/${tokenId}`, { 
            method: "PUT", 
            headers: { "Content-Type": "application/json" }, 
